...
export class Stream<T> {
  ...
  public map<R>(fn: (value: T, index?: number, array?: T[]) => R): Stream<R> {
    ...
    return Stream.of<R>(this.value.map(fn));
  }
  ...
  public collectToJson(fn: (value: T, index?: number, array?: T[]) => string): { [key: string]: any } {
    Assert.nonNull(fn, 'stream:collectToJson:fn must not be null');
    const result = {};
    this.value.forEach(((value, index, array) => {
      const key = fn(value, index, array);
  ...
  public findFirst(): Optional<T> {
    return Optional.ofNullable<T>(this.value[0]);
  }

  public findLast(): Optional<T> {
    return Optional.ofNullable<T>(this.value[this.value.length - 1]);
  }

  public findAny(): Optional<T> {
    return Optional.ofNullable<T>(this.value[NumberUtils.randomInteger(0, this.value.length)]);
  }
